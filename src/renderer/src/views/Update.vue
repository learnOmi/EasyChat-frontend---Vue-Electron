      ...
      <div class="update-inner">
        <div class="update-content">
          ...
          <div class="update-list">
            <div v-for="(item, index) in updateList" :key="index">{{ item }}</div>
          </div>
        </div>
        <div v-if="downloading" class="download-progress">
          <div v-if="downloadPercent.progress != 100">
            ...
            <div class="download-tips">
              正在下载更新包，请稍候({{ proxyRefs.Utils.size2Str(downloadPercent.loaded) }}) /
              {{ proxyRefs.Utils.size2Str(downloadPercent.total) }}
            </div>
          </div>
          <div v-else>更新包下载完成，正在安装，请稍候...</div>
        </div>
...
const checkUpdateAuto = async (auto) => {
  let result = await proxy.Request({
    ...
    params: {
      ...
      uid: userInfoStroe.getUserInfo().userId
    }
  })
  if (!result) {
    return
...
onMounted(() => {
  ...
  window.electron.ipcRenderer.on('downloadUpdateCallback', (event, loaded) => {
...
onUnmounted(() => {
  window.electron.ipcRenderer.removeAllListeners('downloadUpdateCallback')
})
</script>
